(function() {
    ...
    function loadAdvancedSettings() {
        try {
            var saved = localStorage.getItem('drxaos_advanced_settings');
            if (saved) {
                Object.assign(advancedSettings, JSON.parse(saved));
                // lampaLogger.log('Загружены расширенные настройки');
            }
            
            // Принудительно устанавливаем нормальный режим, если не установлен
            if (!advancedSettings.interfaceSize) {
                advancedSettings.interfaceSize = 'normal';
    ...
    function saveAdvancedSettings() {
        try {
            localStorage.setItem('drxaos_advanced_settings', JSON.stringify(advancedSettings));
...
var isTVDevice = deviceDetection.isTV();
...
if (typeof Lampa !== 'undefined' && Lampa.Listener) {
    Lampa.Listener.follow('back', function() {
        ...
        // Дополнительная защита - если фокус застрял после закрытия модального окна
        var $modal = $('.drxaos-quick-theme-modal');
        if ($modal.length === 0) {
            // Модальное окно закрыто, но возможно фокус застрял
            if (document.activeElement && document.activeElement.blur) {
                document.activeElement.blur();
            }
