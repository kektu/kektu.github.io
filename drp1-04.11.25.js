...
drxaos_btn_trailer: { ru: 'Трейлеры', en: 'Trailers', uk: 'Трейлери' },
        ...
        try {
            styleManager.setStyle('drxaos-details-style', `
                .full-start-new__details {
                    ...
                    border-radius: 999px;
                    background: rgba(var(--primary-rgb, 40, 44, 52), 0.16);
                    border: 1px solid rgba(var(--primary-rgb, 40, 44, 52), 0.32);
                    box-shadow: 0 6px 18px rgba(0, 0, 0, 0.35);
                    color: var(--text-main, #f1f5f9);
                    font-family: 'Netflix Sans', -apple-system, BlinkMacSystemFont, 'Segoe UI', 'Helvetica Neue', Arial, sans-serif;
                    font-size: 0.95em;
                    line-height: 1.4;
                ...
                .full-start-new__details span:last-child:not(.full-start-new__split) {
                    font-weight: 600;
                }
                .full-start-new__details span:first-child::before {
                    content: '📺';
                    font-size: 1.05em;
                }
                .full-start-new__details span:nth-child(3)::before {
                    content: '🎬';
                    font-size: 1.05em;
                }
                .full-start-new__details span:last-child:not(.full-start-new__split)::before {
                    content: '🎭';
                    font-size: 1.05em;
                }
                .full-start-new__details span.full-start-new__split {
                    ...
                    color: var(--theme-accent, #00c8e6);
                    opacity: 0.6;
                    transform: translateY(-1px);
                }
                .full-start-new__details:hover {
...
var BUTTON_ICON_SVGS = {
    online: '<svg data-drxaos-icon="online" width="24" height="24" viewBox="0 0 24 24" fill="none"><path d="M12 5C7 5 2.73 8.11 1 12c1.73 3.89 6 7 11 7s9.27-3.11 11-7c-1.73-3.89-6-7-11-7z" fill="#3B82F6"/><circle cx="12" cy="12" r="3" fill="#1E40AF"/><circle cx="12" cy="12" r="1.5" fill="white"/></svg>',
    torrent: '<svg data-drxaos-icon="torrent" width="24" height="24" viewBox="0 0 24 24" fill="none"><path d="M5 2h5v12a2 2 0 1 0 4 0V2h5v12a7 7 0 1 1-14 0V2z" fill="#DC2626"/><path d="M5 2h5v4H5zM14 2h5v4h-5z" fill="#F87171"/><path d="M5 6h5v3H5zM14 6h5v3h-5z" fill="#F97316"/><path d="M5 2h5v2H5zM14 2h5v2h-5z" fill="#E2E8F0"/></svg>',
    play: '<svg data-drxaos-icon="play" width="24" height="24" viewBox="0 0 24 24" fill="none"><circle cx="12" cy="12" r="10" fill="#EF4444"/><path d="M10 8v8l6-4-6-4z" fill="white"/></svg>',
    trailer: '<svg data-drxaos-icon="trailer" width="24" height="24" viewBox="0 0 24 24" fill="none"><rect x="2" y="5" width="20" height="14" rx="2" fill="#FF9800"/><path d="M10 9v6l5-3-5-3z" fill="white"/><circle cx="6" cy="8" r="1" fill="#FFB74D"/><circle cx="18" cy="8" r="1" fill="#FFB74D"/></svg>',
...
function applyButtonIcons(scope) {
    try {
        ...
        $root.find('.full-start__button.view--online, .lampac--button, .full-start__button.drxaos-btn--online').each(function() {
            var $btn = $(this);
        ...
        $root.find('.full-start__button[class*="play"], .full-start__button.button--play, .full-start__button.drxaos-btn--play').each(function() {
            var $btn = $(this);
            var svg = $btn.find('svg[data-drxaos-icon]').first();
            if (!svg.length) svg = $btn.find('svg').first();
    ...
    try {
        ...
        styleManager.setStyle('drxaos-action-buttons', `
            ...
            .full-start__button.drxaos-btn {
                ...
                border-radius: 18px;
                background: linear-gradient(135deg, rgba(var(--primary-rgb, 48, 65, 124), 0.18), rgba(var(--bg-rgb, 10, 14, 24), 0.32));
                border: 1px solid rgba(var(--primary-rgb, 48, 65, 124), 0.35);
                box-shadow: 0 8px 22px rgba(10, 12, 20, 0.45);
                color: var(--text-main, #f1f5f9);
                font-family: 'Netflix Sans', -apple-system, BlinkMacSystemFont, 'Segoe UI', Helvetica, Neue, Arial, sans-serif;
                font-size: 0.95em;
                font-weight: 600;
            ...
            .full-start__button.drxaos-btn:focus {
                ...
                box-shadow: 0 14px 28px rgba(10, 12, 20, 0.55);
            }
            .full-start__button.drxaos-btn--play {
                background: linear-gradient(135deg, rgba(239, 68, 68, 0.92), rgba(248, 113, 113, 0.78));
            ...
            .full-start__button.drxaos-btn--torrent {
                background: linear-gradient(135deg, rgba(124, 58, 237, 0.25), rgba(45, 212, 191, 0.18));
                border-color: rgba(99, 102, 241, 0.45);
            }
            .full-start__button.drxaos-btn--trailer {
                background: linear-gradient(135deg, rgba(249, 115, 22, 0.22), rgba(253, 186, 116, 0.16));
                border-color: rgba(251, 146, 60, 0.32);
            }
            .full-start__button.drxaos-btn--book {
                background: linear-gradient(135deg, rgba(250, 204, 21, 0.24), rgba(250, 204, 21, 0.12));
                border-color: rgba(250, 204, 21, 0.35);
            }
            .full-start__button.drxaos-btn--reaction {
                background: linear-gradient(135deg, rgba(236, 72, 153, 0.24), rgba(249, 168, 212, 0.18));
                border-color: rgba(236, 72, 153, 0.32);
            }
            .full-start__button.drxaos-btn--subscribe {
                background: linear-gradient(135deg, rgba(34, 197, 94, 0.22), rgba(74, 222, 128, 0.16));
            ...
            .full-start__button.drxaos-btn--options span {
                display: none;
            }
...
function syncActionButtons(context) {
    try {
        ...
        var container = context.querySelector('.full-start__buttons, .full-start-new__buttons');
        if (!container) return;

        var buttons = Array.from(container.querySelectorAll('.full-start__button'));
        if (!buttons.length) return;

        var buttonInfoList = buttons.map(function(btn) {
            var info = {
                btn: btn,
            ...
            if (!info.key && info.span) {
                ...
                else if (text.includes('трейлер')) info.key = 'trailer';
            ...
            if (!info.key) {
                if (btn.classList.contains('button--play')) info.key = 'watch';
                ...
                else if (btn.classList.contains('view--online')) info.key = 'online';
        ...
        if (alreadyProcessed && !needsRepair) return;
...
applyDetailsStyles();
syncActionButtons();
window.drxaosButtonsHandler = function() {
    syncActionButtons();
};
...
(function(){
(function _a(){ const _b = ` /* ловим все варианты контейнеров из тем */ .full-start__buttons, .full-start-new__buttons, .full-start__actions, .full-actions__row, [class*="full-start"][class*="_h"], [class*="full-"][class*="actions"][class*="row"]{ display:flex !important; flex-_r:nowrap !important; justify-content:flex-start !important; align-items:center !important; gap:10px !important; overflow-x:auto !important; overflow-y:hidden !important; padding:0 12px !important; mask-image:none !important; -webkit-mask-image:none !important; min-width:0 !important; } /* выключаем декоративные псевдоэлементы-клипмаски у тем */ .full-start__buttons::before, .full-start__buttons::after, .full-start-new__buttons::before, .full-start-new__buttons::after, .full-start__actions::before, .full-start__actions::after, .full-actions__row::before, .full-actions__row::after{ display:none !important; content:none !important; } /* кнопки — компактные, без автоотступов/роста */ .full-start__button{ flex:0 0 auto !important; margin:0 !important; white-space:nowrap !important; width:auto !important; min-width:auto !important; max-width:none !important; height:44px !important; max-height:44px !important; padding:0 18px !important; border-radius:14px !important; display:inline-flex !important; align-items:center !important; justify-content:center !important; line-height:1.2 !important; _o0-sizing:border-_o0 !important; scroll-snap-align:start; } .full-start__button .full-start__icon{ margin-right:8px !important; } /* глушим любые авторастяжки на потомках */ .full-start__buttons *,.full-start-new__buttons *,.full-start__actions *,.full-actions__row *{ flex-grow:0 !important; flex-shrink:0 !important; margin-left:0 !important; } /* прячем «спейсеры» от тем */ .full-start__buttons > [class*="space"], .full-start__buttons > [class*="spacer"], .full-start__buttons > [class*="delimiter"], .full-start-new__buttons > [class*="space"], .full-start-new__buttons > [class*="spacer"], .full-start-new__buttons > [class*="delimiter"], .full-start__actions > [class*="space"], .full-actions__row > [class*="space"], .full-actions__row > [style*="flex:1"], .full-start__buttons > [style*="flex:1"], .full-start-new__buttons > [style*="flex:1"], .full-start__actions > [style*="flex:1"]{ display:none !important; } /* аккуратный скроллбар; можно скрыть совсем при желании */ .full-start__buttons, .full-start-new__buttons, .full-start__actions, .full-actions__row{ scrollbar-width:thin; } .full-start__buttons::-webkit-scrollbar, .full-start-new__buttons::-webkit-scrollbar, .full-start__actions::-webkit-scrollbar, .full-actions__row::-webkit-scrollbar{ height:6px; } `; (window.styleManager?.setStyle ? styleManager.setStyle('weritos-fix-_h-inline-v5', _b) : document.head.appendChild(Object.assign(document.createElement('style'),{id:'weritos-fix-_h-inline-v5',textContent:_b})) ); function _c(container){ try{ var _d = container && container.closest ? container.closest('.full') : null; var _e = _d ? _d.querySelector('.full-start__tags') : null; var _f = _d ? (_d.querySelector('.full-start__rate, .full-start-new__rate, .full-start__quality, .full-start-new__quality')) : null; if(_e && _f){ var _g = _f.nextElementSibling; if(!(_g && _g.classList && _g.classList.contains('full-start__tags'))){ _f.insertAdjacentElement('afterend', _e); _e.style.display='flex'; _e.style.flexWrap='_r'; _e.style.alignItems='center'; _e.style.gap='.5em'; _e.style.margin='.35em 0 .6em 0'; _e.style.opacity='1'; _e.style.visibility='visible'; _e.style.overflow='visible'; _e.style.webkitMaskImage='none'; _e.style.maskImage='none'; } }
}catch(_e){} if (!container) return; const _h = Array.from(container.querySelectorAll('.full-start__button')); Array.from(container.children).forEach(ch=>{ if (!ch.classList.contains('full-start__button')) ch.remove(); }); _h.forEach(_d0=>{ _d0.style.flex = '0 0 auto'; _d0.style.marginLeft = '0'; _d0.style.width = 'auto'; _d0.style.maxWidth = 'none'; _d0.style.minWidth = 'auto'; if (_d0.parentElement !== container) container.appendChild(_d0); }); Array.from(container.childNodes).forEach(n=>{ if (n.nodeType !== 1) n.parentNode.removeChild(n); else if (!n.classList.contains('full-start__button')) n.remove(); }); container.scrollLeft = 0; if (!container.__wheelBound){ container.addEventListener('wheel', (e)=>{ if (Math.abs(e.deltaX) < Math.abs(e.deltaY)) { container.scrollLeft += e.deltaY; e.preventDefault(); } }, {passive:false}); container.__wheelBound = true; } } function _i(){ document.querySelectorAll( '.full-start__buttons, .full-start-new__buttons, .full-start__actions, .full-actions__row,'+ '[class*="full-start"][class*="_h"], [class*="full-"][class*="actions"][class*="row"]' ).forEach(_c); } const _j = new MutationObserver((m)=> { for (const _k of m) { if (_k.addedNodes && _k.addedNodes.length) { _i(); break; } } }); _j.observe(document.body, {subtree:true, childList:true}); _i(); setTimeout(_i, 200); setTimeout(_i, 600); setTimeout(_i, 1200);
})(); (function(){ 'use strict'; const _l = /android/i.test(navigator.userAgent); function _m(_f0, storageKey, placeholder){ if (document.getElementById('drx-overlay')) return; var _n = (Lampa?.Storage?.get('language') || navigator.language || 'ru').toString().toLowerCase(); _n = _n.startsWith('ru') ? 'ru' : 'en'; var _o = { ru: { save: 'Сохранить', clear: 'Очистить', cancel: 'Отмена' }, en: { save: 'Save', clear: 'Clear', cancel: 'Cancel' } }; var _p = _o[_n]; var _q = (Lampa.Storage.get(storageKey)||'').trim(); var _r = document.createElement('div'); _r.id = 'drx-overlay'; _r.innerHTML = ` <div class="drx-backdrop"></div> <div class="drx-card" role="dialog" aria-label="${_f0}" tabindex="-1"> <div class="drx-_f0">${_f0}</div> <input class="drx-input" type="text" placeholder="${placeholder||''}" value="${_q}" autocomplete="off" autocorrect="off" autocapitalize="off" spellcheck="false"> <div class="drx-row"> <button type="button" class="selector drx-_d0 drx-save" data-act="save">${_p.save}</button> <button type="button" class="selector drx-_d0 drx-clear" data-act="clear">${_p.clear}</button> <button type="button" class="selector drx-_d0 drx-cancel" data-act="cancel">${_p.cancel}</button> </div> </div> `; var _s = document.createElement('style'); _s.id = 'drx-overlay-style'; _s.textContent = ` #drx-overlay{position:fixed;inset:0;z-index:2147483647;pointer-events:auto} #drx-overlay .drx-backdrop{position:absolute;inset:0;background:rgba(0,0,0,.6)} #drx-overlay .drx-card{position:absolute;left:50%;top:50%;transform:translate(-50%,-50%); width:min(92vw,560px);background:#1a1d22;border-radius:16px;_o0-shadow:0 10px 40px rgba(0,0,0,.5); padding:20px;border:1px solid rgba(255,255,255,.08)} #drx-overlay .drx-_f0{font-size:20px;margin-bottom:14px;color:#cfe8ff;text-align:center} #drx-overlay .drx-input{width:100%;height:42px;margin-bottom:12px;border-radius:10px; border:1px solid rgba(255,255,255,.15);background:#111418;color:#e7eef7; padding:0 12px;outline:none} #drx-overlay .drx-row{display:flex;gap:10px;flex-_r:_r} #drx-overlay .selector{ flex:1 1 30%;display:flex;align-items:center;justify-content:center; min-height:42px;border-radius:10px;background:#24303b;color:#e7eef7; border:1px solid rgba(255,255,255,.12);user-select:none;cursor:pointer; transition:background .15s, transform .1s;touch-action:manipulation; -webkit-tap-highlight-color:transparent;appearance:none;-webkit-appearance:none;outline:none } #drx-overlay .selector:hover{background:#2f3b4a} #drx-overlay .selector:active{transform:scale(.97)} #drx-overlay .drx-save{background:#2f6fed} `; document.body.appendChild(_r); document.head.appendChild(_s); var $card = _r.querySelector('.drx-card'); var $input = _r.querySelector('.drx-input'); var $back = _r.querySelector('.drx-backdrop'); var $btns = _r.querySelectorAll('.selector'); let _x = false; function _y(){ document.removeEventListener('click', _c0, true); document.removeEventListener('touchend', _c0, true); _r.remove(); const _z = document.getElementById('drx-overlay-style'); _z && _z.remove(); try { Lampa.Controller.toggle('settings_component'); } catch(e){} _x = true;
} function _a0(act){ if (_x) return; if (act === 'save'){ const _b0 = ($input.value || '').trim(); Lampa.Storage.set(storageKey, _b0); try { Lampa.Noty.show(_f0 + ' — ' + (_b0 || '—')); } catch(e){} } else if (act === 'clear'){ Lampa.Storage.set(storageKey, ''); try { Lampa.Noty.show(_f0 + ' — очищен'); } catch(e){} } setTimeout(_y, 120); } function _c0(e){ if (!_r.contains(e.target)) { e.stopPropagation(); e.preventDefault(); return; } if (e.target === $back){ e.stopPropagation(); e.preventDefault(); _a0('cancel'); return; } const _d0 = e.target.closest('.selector'); if (_d0){ e.stopPropagation(); e.preventDefault(); _a0(_d0.getAttribute('data-act')); return; } if ($card.contains(e.target)){ e.stopPropagation(); }
} document.addEventListener('click', _c0, true);
document.addEventListener('touchend', _c0, true); $input.addEventListener('keydown', (e)=>{ e.stopPropagation(); if (e.key === 'Enter'){ e.preventDefault(); _a0('save'); }
}, true); setTimeout(()=>{ $input.focus(); $input.select(); }, 40);
} window.__drx_openOverlay = _m; })(); (function () { 'use strict'; var _l = /android/i.test(navigator.userAgent); if (!window.Lampa || !Lampa.SettingsApi || typeof Lampa.SettingsApi.addParam !== 'function') return; if (!_l) return; if (Lampa.SettingsApi.__dr_overlay_patched) return; Lampa.SettingsApi.__dr_overlay_patched = true; var _e0 = Lampa.SettingsApi.addParam.bind(Lampa.SettingsApi); Lampa.SettingsApi.addParam = function (cfg) { try { var _f0 = (cfg && cfg.field && cfg.field.name ? String(cfg.field.name) : '').toLowerCase(); var _g0 = cfg.onChange; if (_f0.indexOf('tmdb api key') !== -1 || _f0.indexOf('tmdb ключ') !== -1) { cfg.onChange = function (value) { if (value === 'custom') { window.__drx_openOverlay('TMDB API Key', 'tmdb_api_key', '32-символьный ключ'); return; } if (typeof _g0 === 'function') return _g0.apply(this, arguments); }; } if (_f0.indexOf('jacred url') !== -1 || _f0.indexOf('jacred') !== -1) { cfg.onChange = function (value) { if (value === 'custom') { window.__drx_openOverlay('JacRed URL', 'jacred_url', 'например: jacred.xyz (без http://)'); return; } if (typeof _g0 === 'function') return _g0.apply(this, arguments); }; } } catch (e) { try { console.error('[DRXAOS] android-only overlay patch error', e); } catch (_) {} } return _e0(cfg); }; console.log('[DRXAOS] Android-only overlay enabled (PC untouched)');
}()); (function () { 'use strict'; if (/android/i.test(navigator.userAgent)) return; if (!window.Lampa || !Lampa.SettingsApi || typeof Lampa.SettingsApi.addParam !== 'function') return; if (Lampa.SettingsApi.__drx_pc_guard_installed) return; Lampa.SettingsApi.__drx_pc_guard_installed = true; const _e0 = Lampa.SettingsApi.addParam.bind(Lampa.SettingsApi); const _h0 = [ 'цветовая схема','color scheme', 'свечение','glow', 'анимации','animations' ]; const _i0 = s => String(s||'').trim().toLowerCase().replace(/\s+/g,' '); const _j0 = new Set(); if (Lampa.Listener) { Lampa.Listener.follow('settings_open', () => { _j0.clear(); }); } Lampa.SettingsApi.addParam = function(cfg){ try{ const _k0 = _i0(cfg && cfg.field && (cfg.field.name || cfg.name)); if (_h0.some(k => _k0.indexOf(k) !== -1)) { if (_j0.has(_k0)) { return; } _j0.add(_k0); } } catch(e) {} return _e0(cfg); }; if (Lampa.Listener) { Lampa.Listener.follow('settings_open', function(e){ const _l0 = e.body || document; const _m0 = new Set(); _l0.querySelectorAll('.settings-param').forEach(row=>{ const _f0 = _i0(row.querySelector('.settings-param__name')?.textContent || ''); if (!_h0.some(k => _f0.indexOf(k) !== -1)) return; if (_m0.has(_f0)) row.remove(); else _m0.add(_f0); }); }); } console.log('[DRXAOS] PC guard: dedupe for scheme/glow/animations active');
})(); (function () { 'use strict'; var _s = ` body .console .head-backward__button{ position:absolute !important; top:8px !important; left:8px !important; background: rgba(255, 64, 64, 0.18) !important; border-radius:50% !important; width:2.5em !important; height:2.5em !important; display:flex !important; align-items:center !important; justify-content:center !important; _o0-shadow: var(--perf-shadow) !important; transition: var(--perf-transition) !important; z-index:1001 !important; } /* прячем штатный SVG/стрелку */ body .console .head-backward__button svg{ display:none !important; } /* сам крест */ body .console .head-backward__button::after{ content:'✕'; font-size:18px; font-weight:700; color:#ff4040; line-height:1; } body .console .head-backward__button:hover{ background: rgba(255, 64, 64, 0.32) !important; transform: scale(1.05); } body .console .head-backward__button:active{ transform: scale(0.95); }`; if (!document.getElementById('dr-console-x-style')) { var _z = document.createElement('style'); _z.id = 'dr-console-x-style'; _z.textContent = _s; document.head.appendChild(_z); } function _n0() { var _d0 = document.querySelector('body .console .head-backward__button'); if (!_d0 || _d0.dataset.drCloseArmed) return; _d0.dataset.drCloseArmed = '1'; _d0.addEventListener('click', function (e) { e.stopPropagation(); var _o0 = _d0.closest('.console, .modal, .layer, .window, .dialog'); if (_o0 && _o0.parentNode) _o0.parentNode.removeChild(_o0); }, true); } _n0(); var _p0 = new MutationObserver(_n0); _p0.observe(document.body, { childList: true, subtree: true });
})()})();;
